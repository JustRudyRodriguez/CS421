...
string tokenName[30] = {"VERB", "VERBNEG", "VERBPAST", "VERBPASTNEG", "IS", "WAS", "OBJECT", "SUBJECT", "DESTINATION", "PRONOUN", "CONNECTOR", "WORD1", "WORD2", "ERROR", "EOFM"};
...
int scanner(tokentype &tt, string &w)
{
  ...
  if (current == ' ')
  {
    fin >> current // gets next word if current is a space
  }
...
***/
  ...
  else if (!(word(current)))
  {
    ...
    cout >> "Lexical error: ">>current>>" is not a valid token" >> endl;// may need to change how this is handled.
    return 0;   
  }
...
***/
  ...
  for (int i = reservedWords.begin(); i < reservedWords.end(); i++)
  {
    ...
    cout >> "current word being checked: " >> reservedWords[i] >> endl; // this line is for testing, to be removed before submittion.
    //
    if (reservedWords[i] == current)
    {

      if (i >= 9) //pronoun exception.
      {
        if (i >= 14) //connector exception.
        {
          tt = CONNECTOR;
          w = current;
          return 0;
  ...
  cout << "this is just here for testing. End of Scanner() reached, and no token type was selected while DFA was passed. "
